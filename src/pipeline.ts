...
const pipelineDeduplicator = new RequestDeduplicator<PipelineResponse>();

/**
 * Generate a unique key for a pipeline request
 */
function generatePipelineKey(request: PipelineRequest): string {
...
export async function executePipeline(
  ...
  return pipelineDeduplicator.execute(pipelineKey, () => 
    executePipelineInternal(request)
  );
}
...
async function executePipelineInternal(
  ...
  try {
    ...
    const { stageResult: intentResult, output: intent } = await globalMetrics.measure(
      'pipeline.intent',
      () => executeStage('intent', () => analyzeIntent(request.query)),
      { pipelineId }
    );
    ...
    const { stageResult: decomposeResult, output: decomposition } = await executeStage(
      'decompose',
      () => decomposeTask(context)
    );
    ...
    const { stageResult: variantsResult, output: variantData } = await executeStage(
      'variants',
      () => generateVariants(context, decomposition)
    );
    ...
    // Stage 6: Critique
    const { stageResult: critiqueResult, output: critiqueData } = await executeStage(
      'critique',
      () => critiqueVariants(variantData.variants)
    );
    ...
    const bestVariant = variantData.variants.find(
      (v) => v.id === critiqueData.bestVariantId
    );
    const bestCritique = critiqueData.critiques.find(
      (c) => c.variantId === critiqueData.bestVariantId
    );

    if (!bestVariant || !bestCritique) {
      throw createPipelineError(
        'OPTIMIZATION_FAILED',
        'Could not find best variant for optimization',
        'optimize'
      );
    }

    const { stageResult: optimizeResult, output: optimizedOutput } = await executeStage(
      'optimize',
      () => optimizeVariant(bestVariant, bestCritique)
    );
    ...
    const pipelineError = error && typeof error === 'object' && 'code' in error
      ? error as { code: string; message: string; stage?: string }
      : {
          code: 'PIPELINE_ERROR',
    ...
    return {
      ...
      error: {
        ...
        stage: pipelineError.stage as any,
      },
    };
  }
}
 ...
 */
function createSkippedStage(name: StageResult['name']): StageResult {
  return {
    name,
    success: true,
    durationMs: 0,
...
export async function executeLightPipeline(
  ...
  try {
    // Stage 1: Intent Analysis
    const { stageResult: intentResult, output: intent } = await executeStage(
      'intent',
      () => analyzeIntent(request.query)
    );
    stages.push(intentResult);
    ...
    const { stageResult: contextResult, output: context } = await executeStage(
      'context',
      () => buildContext(intent, request.repoPath)
    );
    stages.push(contextResult);
    ...
    const simpleDecomposition: DecompositionResult = {
    ...
    const { stageResult: variantsResult, output: variantData } = await executeStage(
      'variants',
      () => generateVariants(context, simpleDecomposition, 1)
    );
    stages.push(variantsResult);
 ...
 */
export async function executePipelineWithProgress(
  request: PipelineRequest,
  onProgress: (stage: string, status: 'started' | 'completed' | 'failed', data?: unknown) => void
): Promise<PipelineResponse> {
  const startTime = performance.now();
  const startedAt = nowISO();
  const stages: StageResult[] = [];
  ...
  try {
    ...
    if (!intent) throw new Error('Intent analysis failed');
    ...
    const { output: context } = await runStage(
      'context',
      () => buildContext(intent, request.repoPath, gitContext ?? undefined)
    );
    ...
    if (!decomposition) throw new Error('Decomposition failed');
    stages.push({ name: 'decompose', success: true, durationMs: 0 });

    const { output: variantData } = await runStage(
      'variants',
      () => generateVariants(context, decomposition)
    );
    if (!variantData) throw new Error('Variant generation failed');
    stages.push({ name: 'variants', success: true, durationMs: 0 });

    const { output: critiqueData } = await runStage(
      'critique',
      () => critiqueVariants(variantData.variants)
    );
    if (!critiqueData) throw new Error('Critique failed');
    ...
    if (!bestVariant || !bestCritique) throw new Error('Best variant not found');

    const { output: optimized } = await runStage(
      'optimize',
      () => optimizeVariant(bestVariant, bestCritique)
    );
 ...
 */
export function validateRequest(request: unknown): request is PipelineRequest {
 ...
 */
export function getPipelineStatus(response: PipelineResponse): {
