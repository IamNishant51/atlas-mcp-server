 ...
 */
export async function optimizeVariant(
 ...
 */
function hasCriticalIssues(critique: Critique): boolean {
 ...
 */
function normalizeOptimizationType(type: string): Optimization['type'] {
  const normalized = type.toLowerCase();
  if (normalized.includes('perform')) return 'performance';
  if (normalized.includes('read') || normalized.includes('maintain')) return 'readability';
  if (normalized.includes('secur')) return 'security';
  if (normalized.includes('simpl')) return 'simplification';
 ...
 */
function normalizeImpact(impact: string): Optimization['impact'] {
 ...
 */
function estimateFinalMetrics(
  original: CritiqueAssessment,
 ...
 */
export async function multiPassOptimization(
  variant: SolutionVariant,
  critique: Critique,
  maxPasses: number = 2
): Promise<OptimizedOutput> {
  ...
  for (let pass = 0; pass < maxPasses; pass++) {
    ...
    const currentVariant: SolutionVariant = {
      ...variant,
    ...
    logger.debug(
      { pass: pass + 1, qualityScore: currentCritique.qualityScore },
...
export const OptimizationPresets = {
   ...
   */
  performance: {
    focus: ['performance', 'simplification'] as Optimization['type'][],
   ...
   */
  quality: {
    focus: ['readability', 'best_practice'] as Optimization['type'][],
   ...
   */
  security: {
    focus: ['security'] as Optimization['type'][],
   ...
   */
  balanced: {
    focus: ['performance', 'readability', 'security', 'best_practice'] as Optimization['type'][],
...
export function getOptimizationRecommendations(
  ...
  // Find weakest areas
  const scores = Object.entries(assessment) as [keyof CritiqueAssessment, number][];
