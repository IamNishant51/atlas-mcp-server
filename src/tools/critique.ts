...
export async function critiqueVariants(
  ...
  const bestCritique = viableCritiques.length > 0
    ? viableCritiques.reduce((best, current) =>
        current.qualityScore > best.qualityScore ? current : best
      )
    : critiques.reduce((best, current) =>
        current.qualityScore > best.qualityScore ? current : best
      );
 ...
 */
function buildCritiquePrompt(variant: SolutionVariant): string {
 ...
 */
function fallbackCritique(variant: SolutionVariant): Critique {
  // Basic heuristic analysis
  const hasTypeAnnotations = variant.content.includes(': ');
  const hasErrorHandling = 
    variant.content.includes('try') || 
  ...
  const assessment: CritiqueAssessment = {
 ...
 */
function normalizeAssessment(assessment: Record<string, number>): CritiqueAssessment {
  const normalize = (value: number) => Math.min(100, Math.max(0, Math.round(value)));
 ...
 */
function normalizeSeverity(severity: string): CritiqueIssue['severity'] {
 ...
 */
function normalizeCategory(category: string): keyof CritiqueAssessment {
  const normalized = category.toLowerCase();
  if (normalized.includes('correct')) return 'correctness';
  if (normalized.includes('perform')) return 'performance';
  if (normalized.includes('maintain') || normalized.includes('read')) return 'maintainability';
...
function calculateQualityScore(
  ...
  let score = Object.entries(assessment).reduce(
    (total, [key, value]) => total + value * (weights[key as keyof typeof weights] ?? 0.1),
    0
  );
 ...
 */
function generateCritiqueSummary(critiques: Critique[]): string {
  ...
  const criticalIssues = critiques.reduce(
 ...
 */
export function getCommonIssues(critiques: Critique[]): {
  category: keyof CritiqueAssessment;
  count: number;
  examples: string[];
}[] {
  const issuesByCategory = new Map<keyof CritiqueAssessment, string[]>();
 ...
 */
export function getImprovementPriorities(critiques: Critique[]): {
  area: keyof CritiqueAssessment;
  averageScore: number;
  priority: 'high' | 'medium' | 'low';
}[] {
  const areas: (keyof CritiqueAssessment)[] = [
    'correctness',
    'performance',
    'maintainability',
    'security',
  ...
  return areas
    .map((area) => {
      ...
      else if (averageScore < 70) priority = 'medium';
      else priority = 'low';
 ...
 */
export function compareCritiques(a: Critique, b: Critique): {
  winner: string;
  margin: number;
  comparison: Record<keyof CritiqueAssessment, 'a' | 'b' | 'tie'>;
} {
  const comparison = {} as Record<keyof CritiqueAssessment, 'a' | 'b' | 'tie'>;
  
  for (const key of Object.keys(a.assessment) as (keyof CritiqueAssessment)[]) {
    const diff = a.assessment[key] - b.assessment[key];
    if (diff > 5) comparison[key] = 'a';
    else if (diff < -5) comparison[key] = 'b';
    else comparison[key] = 'tie';
  }
