...
import { logger, isDefined, LRUCache, parallelMap, generateCacheKey, globalMetrics } from '../utils.js';
...
const projectCache = new LRUCache<string, ProjectInfo>(PROJECT_CACHE_SIZE, PROJECT_CACHE_TTL_MS);

/** Cache for file existence checks */
const fileExistsCache = new LRUCache<string, boolean>(200, 30000); // 30s TTL
...
export async function buildContext(
  ...
  try {
    ...
    context.projectInfo = await globalMetrics.measure(
      'context_analyze_project',
      () => analyzeProject(normalizedPath)
    );

    // Find relevant code snippets in parallel with project analysis
    context.codeSnippets = await globalMetrics.measure(
      'context_find_snippets',
      () => findRelevantSnippets(normalizedPath, intent, context.projectInfo!)
    );
...
export async function analyzeProject(rootPath: string): Promise<ProjectInfo> {
  ...
  const configChecks = await parallelMap(
    CONFIG_FILES,
    async (configFile) => {
  ...
  if (await fileExists(packageJsonPath)) {
    try {
      ...
      const frameworkDetectors: Array<[string, string]> = [
        ['@nestjs/core', 'NestJS'],
        ['next', 'Next.js'],
        ['react', 'React'],
        ['vue', 'Vue'],
      ...
      for (const [pkg, framework] of frameworkDetectors) {
        if (allDeps[pkg]) frameworks.add(framework);
      }
...
async function findRelevantSnippets(
  ...
  if (targetFiles.length === 0) {
    const relevantFiles = await scanForRelevantFiles(
      rootPath,
      intent.keywords,
      5
    );
 ...
 */
async function scanForLanguages(
  dir: string,
 ...
 */
async function findFile(rootPath: string, fileName: string): Promise<string | null> {
