...
export class OllamaClient {
  ...
  private async doInitialize(): Promise<void> {
    try {
      ...
      // Check if the specified model is available
        ...
        this.config.model = models[0]!;
        logger.warn(
          { requested: originalModel, using: this.config.model, available: models },
          'Requested model not found, falling back to first available model'
        );
      }

      logger.info(
        { baseUrl: this.config.baseUrl, model: this.config.model, availableModels: models },
  ...
  async generateJson<T>(
    prompt: string,
    options: GenerationOptions = {}
  ): Promise<{ data: T | null; raw: string; stats: GenerationResponse['stats'] }> {
